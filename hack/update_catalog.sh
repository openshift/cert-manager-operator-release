...
replicate_catalog_bundle()
{
	...
	if [[ "${REPLICATE_BUNDLE_FILE_IN_CATALOGS}" == "yes" ]]; then
		...
		done < <(find catalogs/*/catalog/openshift-cert-manager-operator -type d ! -path "${CATALOG_DIR}/*")
		return
	fi
	...
	for ocp_version in "${ocp_versions[@]}"; do
		...
		target_dir="${target_catalog_dir}/${CERT_MANAGER_CATALOG_NAME}"
		if [[ "${target_dir}" == "${CATALOG_DIR}/${CERT_MANAGER_CATALOG_NAME}" ]]; then
